...
export default function StudentExamsPage({ onBack, onStartExam, grade = 'Grade 7', classId }: StudentExamsPageProps) {
  ...
  useEffect(() => {
    // Determine if we should auto-load based on stored student data or initial grade
  ...
  const fetchExamsAndAttempts = async () => {
    ...
    try {
      ...
      // Use the searchGrade state, or fallback to student's grade if searchGrade is empty
      const effectiveGrade = searchGrade || student?.grade || grade;

      // Seed prebuilt if necessary
      await examService.seedPrebuiltExams();

      // We search by passing separate fields to the service
      const examData = await examService.searchExams(effectiveGrade, searchTeacher, searchSchool);
      setExams(examData);
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="bg-brand-bg/50 rounded-2xl p-4 border border-brand-border flex flex-col gap-1 items-start">
                         ...
                         <span className="text-lg font-black text-brand-text tracking-tighter">{exam.questions.reduce((s,q) => s + q.marks, 0)} Pts</span>
                      </div>
                    </div>
                    ...
                    {isCompleted ? (
                      ...
                      <button
                        ...
                        className={`w-full group flex items-center justify-center gap-3 py-5 rounded-[2rem] text-white font-black uppercase text-xs tracking-widest shadow-2xl transition-all active:scale-[0.97] hover:brightness-105 ${statusColor} shadow-${statusColor.split('-')[1]}-500/25`}
                      >
                        {statusIcon}
                        {statusText}
